...
- `[TELEFONNUMMER]` — erscheint zweimal (Hero ist ok ohne, aber im Kontakt-Bereich bitte ersetzen, inkl. `tel:`-Link)
- `[ÖFFNUNGSZEITEN]` — im Standort-Bereich
...
1. Neues Repository auf GitHub erstellen, z. B. `feros-barbier`
2. Diesen Ordnerinhalt hochladen (entweder per Drag&Drop im Browser oder per Git):
   ...
   git branch -M main
   git remote add origin https://github.com/DEIN-USERNAME/feros-barbier.git
...
5. Nach 1–2 Minuten ist die Seite live unter:
   ```
   https://DEIN-USERNAME.github.io/feros-barbier/
   ```

## Später: eigene Domain
Wenn Fero später eine eigene Domain möchte (z. B. `feros-barbier.de`), kann die einfach
